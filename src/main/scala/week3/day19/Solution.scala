...
object Solution {
      ...
      .map { case ruleIdPattern(idStr, rest) =>
        ...
        rest match {
          ...
          case ruleCompositionPattern(rules1, rules2) =>
            id -> ComposedRule(ruleIdList(rules1), ruleIdList(rules2))
          case _ =>
  ...
  private def validMatches(rules: Map[Int, Rule], rule: Rule, str: String): List[Matched] = {
    if (str == "") {
      ...
      rule match {
        ...
        case ComposedRule(rules1, rules2) =>
  ...
  private def solvePartTwo(rules: Map[Int, Rule], toValidate: List[String]): Int = {
    val cyclicRules = rules ++ Map(
      8  -> ComposedRule(List(42, 8), List(42)),
      11 -> ComposedRule(List(42, 11, 31), List(42, 31))
    )
  ...
  case class ComposedRule(rules1: List[Int], rules2: List[Int]) extends Rule
